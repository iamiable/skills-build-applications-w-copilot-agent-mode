...
router.post('/', async (req, res) => {
    try {
        ...
        const workout = await Workout_1.default.create({ userId, name, exercises, duration });
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const workout = await Workout_1.default.findByIdAndUpdate(id, { name, exercises, duration }, { new: true }).populate('userId');
        if (!workout) {
            return res.status(404).json({ message: `Workout ${id} not found` });
        }
        res.json({
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const workouts = await Workout_1.default.find({ userId }).populate('userId');
        res.json({
